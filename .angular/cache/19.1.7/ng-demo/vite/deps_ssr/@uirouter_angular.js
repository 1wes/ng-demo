  ...
  function() {
    ...
    UrlMatcher2.prototype.format = function(values2) {
      ...
      var pathString = pathSegmentsAndParams.reduce(function(acc, x) {
        ...
        if (isArray(encoded)) return acc + map(encoded, UrlMatcher2.encodeDashes).join("-");
...
var UIRouterRx = (
  ...
  function() {
    function UIRouterRx2(router) {
      ...
      this.deregisterFns = [];
      var start$ = new import_rxjs.ReplaySubject(1);
      var success$ = start$.pipe((0, import_operators.mergeMap)(function(t) {
        return t.promise.then(function() {
          return t;
        }, function() {
          return null;
        });
      }), (0, import_operators.filter)(function(t) {
        return !!t;
      }));
      var params$ = success$.pipe((0, import_operators.map)(function(transition) {
        return transition.params();
      }));
      var states$ = new import_rxjs.ReplaySubject(1);
...
// node_modules/@uirouter/rx/lib-esm/rx-async-policy.js
  ...
  _invokeUiOnParamsChangedHook($transition$) {
    ...
    if (isFunction(uiOnParamsChanged)) {
      ...
      if (changedToParams.length) {
        ...
        const newValues = filter(toParams, (val2, key) => changedKeys.indexOf(key) !== -1);
...
var UISref = class _UISref {
   ...
   */
  targetState$ = new import_rxjs3.ReplaySubject(1);
  /** @internal */
  _emit = false;
  /** @internal */
  _statesSub;
...
var UISrefStatus = class _UISrefStatus {
  ...
  ngAfterContentInit() {
    const transEvents$ = this._globals.start$.pipe((0, import_operators3.switchMap)((trans) => {
      const event = (evt) => ({
        evt,
        trans
      });
      const transStart$ = (0, import_rxjs3.of)(event("start"));
      const transResult = trans.promise.then(() => event("success"), () => event("error"));
      const transFinish$ = (0, import_rxjs3.from)(transResult);
      return (0, import_rxjs3.concat)(transStart$, transFinish$);
    }));
    const withHostSref = (childrenSrefs) => childrenSrefs.concat(this._hostUiSref).filter(identity).reduce(uniqR, []);
    this._srefs$ = new import_rxjs3.BehaviorSubject(withHostSref(this._srefs.toArray()));
    this._srefChangesSub = this._srefs.changes.subscribe((srefs) => this._srefs$.next(withHostSref(srefs.toArray())));
    const targetStates$ = this._srefs$.pipe((0, import_operators3.switchMap)((srefs) => (0, import_rxjs3.combineLatest)(srefs.map((sref) => sref.targetState$))));
    this._subscription = transEvents$.pipe((0, import_operators3.switchMap)((evt) => {
      return targetStates$.pipe((0, import_operators3.map)((targets) => {
